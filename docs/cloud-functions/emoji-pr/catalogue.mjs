...
export const MAX_ICON_PIXELS = 1_048_576;
export const MAX_ICON_SIDE = 4_096;

export function imageFilename(value) {
  try {
    const url = new URL(value);
    if (url.origin !== 'https://img.pvme.io' || !/^\/images\/[A-Za-z0-9_-]+\.png$/.test(url.pathname) || url.search || url.hash) throw new Error();
    return url.pathname.slice('/images/'.length);
  } catch { fail(400, 'Use the public img.pvme.io PNG URL from the PVME Discord bot.'); }
}

export function decodeIconBytes(bytes, status = 400) {
  const reject = message => fail(status, message);
  if (!Buffer.isBuffer(bytes) || bytes.length > 65536 || bytes.length < 33 || bytes.subarray(0, 8).toString('hex') !== '89504e470d0a1a0a') reject('Icons must be valid PNGs, at most 64 KiB.');
  const width = bytes.readUInt32BE(16), height = bytes.readUInt32BE(20);
  if (!width || !height || width > MAX_ICON_SIDE || height > MAX_ICON_SIDE || width * height > MAX_ICON_PIXELS) {
    reject('Icons must be reasonably sized PNGs (up to 4,096 pixels per side and 1,048,576 pixels total).');
  }
  let png;
  try { png = PNG.sync.read(bytes, { checkCRC: true }); } catch { reject('Invalid PNG data.'); }
  let visible = 0, transparent = 0;
  for (let i = 3; i < png.data.length; i += 4) { if (png.data[i]) visible++; if (png.data[i] === 0) transparent++; }
  if (!visible || !transparent) reject('An icon must contain visible artwork and a transparent background.');
...
export function decodeIcon(value) {
  ...
  return decodeIconBytes(Buffer.from(value.slice(22), 'base64'));
}
...
export function validateBatch(items, catalogue) {
  ...
  return items.map(item => {
    if (!item || typeof item !== 'object' || Object.keys(item).some(key => !['category', 'name', 'id', 'id_aliases', 'preset_type', 'preset_slot', 'image_url'].includes(key))) fail(400, 'Unexpected icon fields.');
    ...
    return { ...item, name: item.name.trim(), image_filename: imageFilename(item.image_url) };
  });
}

export function appendEntries(text, items) {
  ...
  for (let index = 0; index < catalogue.categories.length; index++) {
    const entries = items.flatMap(item => {
      ...
      const entry = { name: item.name, id: item.id, image: imageFilename(item.image_url) };
